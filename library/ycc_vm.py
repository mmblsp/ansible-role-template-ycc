...
def vm_argument_spec():
    return dict(
        ...
        active_operations_limit_timeout=dict(
...
class YccVM(YC):
    ...
    def active_op_limit_timeout(self, timeout, fn, *args, **kwargs):
        ...
        if timeout is None:
            ...
            retry = False
            while (
                timeout == 0 or (datetime.datetime.now() -
                                 start_time).seconds < timeout
            ):
                try:
                    op = fn(*args, **kwargs)
                    if retry:
    ...
    def _compare_disk(self, disk_id, disk_spec):
        err = list()
        disk = MessageToDict(self.disk_service.Get(
            GetDiskRequest(disk_id=disk_id)))
    ...
    def _is_same(self, instance: dict, spec: dict):
        ...
        err.extend(self._compare_disk(
            instance["bootDisk"]["diskId"], boot_disk_spec))
    ...
    def _translate(self):
        ...
        for key in params:
            if key in ["memory", "disk_size"]:
                ...
                params[key] = getattr(
                    PlatformId, params[key].replace(" ", "")).value
    ...
    def _get_instance_params(self, spec):  # pylint: disable=R0914
        ...
        hostname = spec.get("hostname") if spec.get(
            "hostname") else spec.get("name")
        ...
        if snapshot_id:
            try:
                self.snapshot_service.Get(
                    GetSnapshotRequest(snapshot_id=snapshot_id))
            except _InactiveRpcError as err:
                raise ValueError(
                    f"Snapshot with id:{snapshot_id} not found") from err
